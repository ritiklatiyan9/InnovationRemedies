...
export default function WorldMap({
  ...
  // Memoize the projected points to avoid recalculation
  const projectedDots = useMemo(() => 
    dots.map(dot => ({
      startPoint: projectPoint(dot.start.lat, dot.start.lng),
      endPoint: projectPoint(dot.end.lat, dot.end.lng),
      path: null // Will be calculated below
    })),
  [dots, projectPoint]);

  // Calculate paths once points are projected
  const paths = useMemo(() => 
    projectedDots.map(dot => ({
      ...dot,
      path: createCurvedPath(dot.startPoint, dot.endPoint)
    })),
